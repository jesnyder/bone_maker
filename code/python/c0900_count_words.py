...
def count_words():
    ...
    for file in os.listdir(path):
        ...
        if 2 in tasks:count_words_each_patent(col_name, terms)

        # count frequency of scaffold design elements per year
        if 3 in tasks: yearly_term_count(col_name, terms)
        if 4 in tasks: cumulative_term_count(col_name, terms)
        if 5 in tasks: yearly_term_percent(col_name, terms)

        # plot counts - yearly and cumulative
        if 6 in tasks: plot_term_count(col_name, terms)
        if 7 in tasks: plot_term_cumulative(col_name, terms)

        # bar chart / flow chart frequency
        if 8 in tasks: plot_bar_percentage(col_name, terms)
...
def plot_term_count(col_name, terms):
    ...
    plt.yscale('log')
    plt.title(col_name)
    plt.legend(bbox_to_anchor=(-0.22, 0.7), loc ="upper left")
